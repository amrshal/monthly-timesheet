...
public final class StatusTransitionPolicy {
    ...
    public static boolean employeeCanEdit(TimesheetStatus status, boolean owner) {
        return owner && status == TimesheetStatus.DRAFT;
    }

    public static boolean privilegedCanEdit(UserRole role, boolean scoped) {
        return scoped && (role == UserRole.MANAGER || role == UserRole.ADMIN);
    }
}
